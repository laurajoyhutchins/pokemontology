...
def build_graph_from_csv(source_dir: Path) -> Graph:
    ...
    for filename, (class_name, rdf_class) in ENTITY_FILES.items():
        ...
        for row in rows:
            add_named_entity(g, class_name, rdf_class, row, filename.removesuffix(".csv"))

    variant_rows = entity_rows["variants.csv"]
    require_columns(source_dir / "variants.csv", variant_rows, ("identifier", "name", "species_identifier"))
    add_variant_links(g, variant_rows)

    version_group_rows = read_rows(source_dir / "version_groups.csv")
    require_columns(source_dir / "version_groups.csv", version_group_rows, ("identifier", "name"))
    ...
    for row in optional_rows.get("typing_assignments.csv", []):
        ...
        g.add((assignment_iri, PKM.aboutVariant, iri_for("Variant", row["variant_identifier"])))
        g.add((assignment_iri, PKM.aboutType, iri_for("Type", row["type_identifier"])))
        g.add((assignment_iri, PKM.hasContext, iri_for("Ruleset", row["version_group_identifier"])))
        g.add((assignment_iri, PKM.hasTypeSlot, Literal(int(row["type_slot"]), datatype=XSD.integer)))
    ...
    for row in optional_rows.get("ability_assignments.csv", []):
        ...
        g.add((assignment_iri, PKM.aboutVariant, iri_for("Variant", row["variant_identifier"])))
        g.add((assignment_iri, PKM.aboutAbility, iri_for("Ability", row["ability_identifier"])))
        g.add((assignment_iri, PKM.hasContext, iri_for("Ruleset", row["version_group_identifier"])))
        g.add((assignment_iri, PKM.isHiddenAbility, Literal(row["is_hidden_ability"].lower() == "true", datatype=XSD.boolean)))
    ...
    for row in optional_rows.get("stat_assignments.csv", []):
        ...
        g.add((assignment_iri, PKM.hasContext, iri_for("Ruleset", row["version_group_identifier"])))
        g.add((assignment_iri, PKM.hasValue, Literal(int(row["value"]), datatype=XSD.integer)))
    ...
    for row in optional_rows.get("move_property_assignments.csv", []):
        ...
        g.add((assignment_iri, PKM.hasContext, iri_for("Ruleset", row["version_group_identifier"])))
        g.add((assignment_iri, PKM.hasMoveType, iri_for("Type", row["move_type_identifier"])))
        g.add((assignment_iri, PKM.hasBasePower, Literal(int(row["base_power"]), datatype=XSD.integer)))
        g.add((assignment_iri, PKM.hasAccuracy, Literal(int(row["accuracy"]), datatype=XSD.integer)))
        g.add((assignment_iri, PKM.hasPP, Literal(int(row["pp"]), datatype=XSD.integer)))
        g.add((assignment_iri, PKM.hasPriority, Literal(int(row["priority"]), datatype=XSD.integer)))
    ...
    for row in optional_rows.get("move_learn_records.csv", []):
        ...
        g.add((assignment_iri, PKM.aboutVariant, iri_for("Variant", row["variant_identifier"])))
        g.add((assignment_iri, PKM.learnableMove, iri_for("Move", row["move_identifier"])))
        g.add((assignment_iri, PKM.hasContext, iri_for("Ruleset", row["version_group_identifier"])))
        g.add((assignment_iri, PKM.isLearnableInRuleset, Literal(row["is_learnable"].lower() == "true", datatype=XSD.boolean)))
    ...
    for row in optional_rows.get("type_effectiveness_assignments.csv", []):
        ...
        g.add((assignment_iri, PKM.attackerType, iri_for("Type", row["attacker_type_identifier"])))
        g.add((assignment_iri, PKM.defenderType, iri_for("Type", row["defender_type_identifier"])))
        g.add((assignment_iri, PKM.hasContext, iri_for("Ruleset", row["version_group_identifier"])))
        g.add((assignment_iri, PKM.hasDamageFactor, Literal(row["damage_factor"], datatype=XSD.decimal)))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--source-dir", type=Path, default=DEFAULT_SOURCE_DIR, help="Directory containing normalized Veekun CSV export files.")
    parser.add_argument("-o", "--output", type=Path, default=DEFAULT_OUTPUT, help="Output TTL path.")
    return parser
