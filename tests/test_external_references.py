...
def test_fixture_uses_external_reference_nodes_for_pokemonkg() -> None:
    graph = Graph()
    graph.parse(REPO / "examples" / "fixtures" / "froakie-caterpie-seed.ttl", format="turtle")

    refs = list(graph.subjects(predicate=PKM.describedByArtifact, object=PKM.DatasetArtifact_PokemonKG))
    assert refs, "Expected at least one PokemonKG external reference node in the fixture."
    for ref in refs:
