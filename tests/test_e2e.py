...
def test_web_page_controller_prefers_actual_pokeapi_source(tmp_path: Path) -> None:
    ...
    # learnset shards are all in the unchecked archive group and must not load by default
    assert not any("mechanics-learnsets" in source for source in payload["sourceCalls"])
